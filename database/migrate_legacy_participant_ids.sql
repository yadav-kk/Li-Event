-- 1. Create a CTE to calculate correct sequential serial numbers per event based on creation order
...
-- 2. Update the students table unique student_id column
UPDATE public.students s
SET student_id = f.new_id
FROM formatted_ids f
WHERE s.id = f.s_uuid;

-- 3. Update the event_participants table registration_number column
